...
export function create_distribution_schedule(distribution_schedule_raw: DistributionScheduleRaw): DistributionSchedule {
	const start_time_secs = to_utc_seconds(distribution_schedule_raw.start_date);
	const end_time_secs = to_utc_seconds(distribution_schedule_raw.end_date);
...
export async function upload_staking_contract(lcd_client: LCDClient, sender: Wallet) {
	const code_id = await store_contract(lcd_client, sender, staking_contract_wasm);
...
export async function init_lp_staking_contract(lcd_client: LCDClient, sender: Wallet, config: Config, code_id: number) {
	const distribution_schedule = create_distribution_schedule(config.distribution_schedule);
	// instantiate lp tokens staking contract
	const staking_config = StakingConfig(config.multisig_address, config.psi_token_addr, config.lp_token_addr, config.terraswap_factory_addr, distribution_schedule);
