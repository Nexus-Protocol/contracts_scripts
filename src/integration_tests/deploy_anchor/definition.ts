...
export async function anchor_init(lcd_client: LCDClient, sender: Wallet) {
	...
	const anchor_token_config = {
		name: "Anchor Terra USD",
		symbol: "aUST",
		decimals: 6,
		initial_balances: [],
	};
	const _anc_token_addr = await init_anc_token(lcd_client, sender, cw20_code_id, anchor_token_config);
	console.log(`=======================`);

	console.log(`Instantiating Anchor contracts...\n\t`);

	//instantiate Market
	const anchor_market_code_id = await store_contract(lcd_client, sender, anchor_market_wasm);
	console.log(`anchor_market uploaded\n\tcode_id: ${anchor_market_code_id}`);
	const anchor_market_config = AnchorMarkerConfig(sender, cw20_code_id);

	const anchor_market_addr = await instantiate_contract(
		lcd_client,
		sender,
	...
	console.log(`=======================`);
	//instantiate oracle
	const anchor_oracle_config = AnchorOracleConfig(sender);
	const anchor_oracle_addr = await create_contract(lcd_client, sender, "anchor_oracle", anchor_oracle_wasm, anchor_oracle_config);
	console.log(`=======================`);
	//instantiate liquidation
	const anchor_liquidation_config = AnchorLiquidationConfig(sender, anchor_oracle_addr);
	const anchor_liquidation_addr = await create_contract(lcd_client, sender, "anchor_liquidation", anchor_liquidation_wasm, anchor_liquidation_config);
	console.log(`=======================`);
	//instantiate distribution
	const anchor_distribution_model_config = AnchorDistrConfig(sender);
	const anchor_distribution_model_addr = await create_contract(lcd_client, sender, "anchor_distribution_model", anchor_distribution_model_wasm, anchor_distribution_model_config);
	console.log(`=======================`);
	//instantiate overseer
	const anchor_overseer_config = AnchorOverseerConfig(sender, anchor_liquidation_addr, anchor_market_addr, anchor_oracle_addr);
	const anchor_overseer_addr = await create_contract(lcd_client, sender, "anchor_overseer", anchor_overseer_wasm, anchor_overseer_config);
	console.log(`=======================`);

	//instantiate interest model
	const anchor_interest_model_config = AnchorInterstConfig(sender);
	const anchor_interest_model_addr = await create_contract(lcd_client, sender, "anchor_interest_model", anchor_interest_model_wasm, anchor_interest_model_config);
	console.log(`=======================`);
